...
public class ObjectiveSystemE : MonoBehaviour
{
    private List<GameObject> enemies = new List<GameObject>();

    [SerializeField] private EUIController uiControllerE;
    ...
    void Update()
    {
        ...
        if (enemies.Count == 0)
        {
            if (uiControllerE != null)
            {
                uiControllerE.ShowCongratulatoryMessage(); // Call the UI controller to show the message
            }
            else
            {
                Debug.LogWarning("UIControllerE reference is missing!");
            }
